...
import { INK, MUTED, CARD, MONO, DANGER } from "../../theme";
  ...
  return (
    <div className="rounded-2xl p-6 flex flex-col" style={{ ...CARD, minHeight: 220 }}>
      <div className="flex items-start justify-between mb-2">
        <p className="text-sm font-medium" style={{ color: INK }}>Device Health</p>
        <button style={{ color: MUTED }}>
          <MoreHorizontal size={18} />
        </button>
      </div>
      <p className="text-2xl font-semibold mb-1" style={{ fontFamily: MONO, color: INK }}>22 flagged</p>
        ...
        <div className="space-y-2">
          {HEALTH_DATA.map((d) => (
            <div key={d.name} className="flex items-center gap-2 text-[13px]">
              ...
              <span className="font-semibold" style={{ color: INK, fontFamily: MONO }}>{d.value}</span>
            </div>
          ))}
        </div>
      </div>
