...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      >
        <img src={piaLogo} alt="PIA Logo" className="w-full h-full object-cover" style={{ objectPosition: "top center" }} />
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6 mt-2">
        {NAV_GROUPS.map((group) => (
          <div key={group.label}>
            {!collapsed && (
              <p className="px-3 mb-2 text-[10.5px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: MUTED }}>
                {group.label}
              </p>
            )}
